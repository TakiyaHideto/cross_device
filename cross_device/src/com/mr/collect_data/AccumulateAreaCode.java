...
/**
 * Created by TakiyaHideto on 16/4/25.
 */
public class AccumulateAreaCode {
    ...
    public static class AccumulateReducer extends Reducer<Text, Text, NullWritable, Text> {
        ...
        protected void reduce(Text key, Iterable<Text> values,Context context)
            ...
            HashMap<String,HashSet<String>> areaCodeTimeMap = new HashMap<String,HashSet<String>>();
            ...
            for (Text value: values){//
                ...
                String[] areaCodeList = elements[2].split(Properties.Base.CTRL_B,-1);
                for (String areaCodeInfo: areaCodeList){
                    String areaCode = areaCodeInfo.split(Properties.Base.CTRL_C,-1)[0];
                    String timestamp = areaCodeInfo.split(Properties.Base.CTRL_C,-1)[2];
                    String date = "";
                    String hour = "";
                    try{
                    ...
                    if (areaCodeTimeMap.size()<1000) {
                        CommUtil.addInfoToMapWithSet(areaCodeTimeMap, areaCode, date + Properties.Base.CTRL_D + hour);
                        CommUtil.addInfoToMap(timeFreqMap, date + Properties.Base.CTRL_D + hour);
                    }
                }
            }
            String areaCodeInfoString = "";
            for (String areaCode: areaCodeTimeMap.keySet()){
                String timeString = "";
                for (String time: areaCodeTimeMap.get(areaCode)){
                    timeString = time + Properties.Base.CTRL_D + String.valueOf(timeFreqMap.get(time));
                    areaCodeInfoString += areaCode + Properties.Base.CTRL_C + timeString + Properties.Base.CTRL_B;
                }
            }

            //
            if (!areaCodeInfoString.equals("")) {
                areaCodeInfoString = areaCodeInfoString.substring(0, areaCodeInfoString.length() - 1);
            }
            if (!accumualteInfo.equals("")){
                accumualteInfo = accumualteInfo.substring(0,accumualteInfo.length()-"accumulate_info".length()-1);
            }
            String valueResult;
            if (!areaCodeInfoString.equals("") && !accumualteInfo.equals("")){
                valueResult = areaCodeInfoString + Properties.Base.CTRL_B + accumualteInfo;
            } else if (!areaCodeInfoString.equals("") && accumualteInfo.equals("")){
                valueResult = areaCodeInfoString;
            } else if (areaCodeInfoString.equals("") && !accumualteInfo.equals("")){
    ...
    public static void main(String[] args) throws Exception{
        ...
        Job job = new Job(conf,"AccumulateAreaCode");
...
//        job.getConfiguration().set("mapreduce.map.java.opts ", "-Djava.net.preferIPv4Stack=true -Xmx3000000000");
        ...
        job.setJarByClass(AccumulateAreaCode.class);
