 ...
 */
public class AccumulateAppType {
    ...
    public static class AccumulateReducer extends Reducer<Text, Text, NullWritable, Text> {
        ...
        protected void reduce(Text key, Iterable<Text> values,Context context)
            ...
            HashMap<String,HashSet<String>> appTypeTimeMap = new HashMap<String,HashSet<String>>();
            ...
            for (Text value: values){//
                ...
                String[] appTypeList = elements[2].split(Properties.Base.CTRL_B,-1);
                for (String appTypeInfo: appTypeList){
                    String appType = appTypeInfo.split(Properties.Base.CTRL_C,-1)[0];
                    String timestamp = appTypeInfo.split(Properties.Base.CTRL_C,-1)[2];
                    String date = "";
                    String hour = "";
                    try{
                    ...
                    if (appTypeTimeMap.size()<1000) {
                        CommUtil.addInfoToMapWithSet(appTypeTimeMap, appType, date + Properties.Base.CTRL_D + hour);
                        CommUtil.addInfoToMap(timeFreqMap, date + Properties.Base.CTRL_D + hour);
                    }
                }
            }
            String appTypeInfoString = "";
            for (String appType: appTypeTimeMap.keySet()){
                String timeString = "";
                for (String time: appTypeTimeMap.get(appType)){
                    timeString = time + Properties.Base.CTRL_D + String.valueOf(timeFreqMap.get(time));
                    appTypeInfoString += appType + Properties.Base.CTRL_C + timeString + Properties.Base.CTRL_B;
                }
            }

            //
            if (!appTypeInfoString.equals("")) {
                appTypeInfoString = appTypeInfoString.substring(0, appTypeInfoString.length() - 1);
            }
            if (!accumualteInfo.equals("")){
                accumualteInfo = accumualteInfo.substring(0,accumualteInfo.length()-"accumulate_info".length()-1);
            }
            String valueResult;
            if (!appTypeInfoString.equals("") && !accumualteInfo.equals("")){
                valueResult = appTypeInfoString + Properties.Base.CTRL_B + accumualteInfo;
            } else if (!appTypeInfoString.equals("") && accumualteInfo.equals("")){
                valueResult = appTypeInfoString;
            } else if (appTypeInfoString.equals("") && !accumualteInfo.equals("")){
    ...
    public static void main(String[] args) throws Exception{
        ...
        Job job = new Job(conf,"AccumulateAppType");
...
//        job.getConfiguration().set("mapreduce.map.java.opts ", "-Djava.net.preferIPv4Stack=true -Xmx3000000000");
        ...
        job.setJarByClass(AccumulateAppType.class);
        ...
        for (int i=0;i<1;i++) {
            CommUtil.addInputFileComm(job, fs, bidLog + "/" + currentData + "/AppType", TextInputFormat.class, SingleDayMapper.class);
