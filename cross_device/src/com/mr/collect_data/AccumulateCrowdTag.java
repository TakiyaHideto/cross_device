...
/**
 * Created by TakiyaHideto on 16/4/26.
 */
public class AccumulateCrowdTag {
    ...
    public static class AccumulateReducer extends Reducer<Text, Text, NullWritable, Text> {
        ...
        protected void reduce(Text key, Iterable<Text> values,Context context)
            ...
            HashMap<String,HashSet<String>> crowdTagTimeMap = new HashMap<String,HashSet<String>>();
            ...
            for (Text value: values){//
                ...
                String[] crowdTagList = elements[2].split(Properties.Base.CTRL_B,-1);
                for (String crowdTagInfo: crowdTagList){
                    String crowdTag = crowdTagInfo.split(Properties.Base.CTRL_C,-1)[0];
                    String timestamp = crowdTagInfo.split(Properties.Base.CTRL_C,-1)[2];
                    String date = "";
                    String hour = "";
                    try{
                    ...
                    if (crowdTagTimeMap.size()<1000) {
                        CommUtil.addInfoToMapWithSet(crowdTagTimeMap, crowdTag, date + Properties.Base.CTRL_D + hour);
                        CommUtil.addInfoToMap(timeFreqMap, date + Properties.Base.CTRL_D + hour);
                    }
                }
            }
            String crowdTagInfoString = "";
            for (String crowdTag: crowdTagTimeMap.keySet()){
                String timeString = "";
                for (String time: crowdTagTimeMap.get(crowdTag)){
                    timeString = time + Properties.Base.CTRL_D + String.valueOf(timeFreqMap.get(time));
                    crowdTagInfoString += crowdTag + Properties.Base.CTRL_C + timeString + Properties.Base.CTRL_B;
                }
            }

            //
            if (!crowdTagInfoString.equals("")) {
                crowdTagInfoString = crowdTagInfoString.substring(0, crowdTagInfoString.length() - 1);
            }
            if (!accumualteInfo.equals("")){
                accumualteInfo = accumualteInfo.substring(0,accumualteInfo.length()-"accumulate_info".length()-1);
            }
            String valueResult;
            if (!crowdTagInfoString.equals("") && !accumualteInfo.equals("")){
                valueResult = crowdTagInfoString + Properties.Base.CTRL_B + accumualteInfo;
            } else if (!crowdTagInfoString.equals("") && accumualteInfo.equals("")){
                valueResult = crowdTagInfoString;
            } else if (crowdTagInfoString.equals("") && !accumualteInfo.equals("")){
    ...
    public static void main(String[] args) throws Exception{
        ...
        Job job = new Job(conf,"AccumulateCrowdTag");
...
//        job.getConfiguration().set("mapreduce.map.java.opts ", "-Djava.net.preferIPv4Stack=true -Xmx3000000000");
        ...
        job.getConfiguration().set("mapreduce.reduce.memory.mb ", "3000");
        job.getConfiguration().set("mapreduce.reduce.java.opts ", "-Djava.net.preferIPv4Stack=true -Xmx3000000000");

        FileSystem fs = FileSystem.get(conf);

        job.setJarByClass(AccumulateCrowdTag.class);
        ...
        for (int i=0;i<1;i++) {
            CommUtil.addInputFileComm(job, fs, bidLog + "/" + currentData + "/AreaCode", TextInputFormat.class, SingleDayMapper.class);
