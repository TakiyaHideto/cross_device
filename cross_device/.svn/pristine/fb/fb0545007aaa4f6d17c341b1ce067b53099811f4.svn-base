...
/**
 * Created by TakiyaHideto on 16/5/5.
 */
public class AccumulateWebType {
    ...
    public static class AccumulateReducer extends Reducer<Text, Text, NullWritable, Text> {
        ...
        protected void reduce(Text key, Iterable<Text> values,Context context)
            ...
            HashMap<String,HashSet<String>> webTypeTimeMap = new HashMap<String,HashSet<String>>();
            ...
            for (Text value: values){//
                ...
                String[] webTypeList = elements[2].split(Properties.Base.CTRL_B,-1);
                for (String webTypeInfo: webTypeList){
                    String webType = webTypeInfo.split(Properties.Base.CTRL_C,-1)[0];
                    String timestamp = webTypeInfo.split(Properties.Base.CTRL_C,-1)[2];
                    String date = "";
                    String hour = "";
                    try{
                    ...
                    if (webTypeTimeMap.size()<1000) {
                        CommUtil.addInfoToMapWithSet(webTypeTimeMap, webType, date + Properties.Base.CTRL_D + hour);
                        CommUtil.addInfoToMap(timeFreqMap, date + Properties.Base.CTRL_D + hour);
                    }
                }
            }
            String webTypeInfoString = "";
            for (String webType: webTypeTimeMap.keySet()){
                String timeString = "";
                for (String time: webTypeTimeMap.get(webType)){
                    timeString = time + Properties.Base.CTRL_D + String.valueOf(timeFreqMap.get(time));
                    webTypeInfoString += webType + Properties.Base.CTRL_C + timeString + Properties.Base.CTRL_B;
                }
            }

            //
            if (!webTypeInfoString.equals("")) {
                webTypeInfoString = webTypeInfoString.substring(0, webTypeInfoString.length() - 1);
            }
            if (!accumualteInfo.equals("")){
                accumualteInfo = accumualteInfo.substring(0,accumualteInfo.length()-"accumulate_info".length()-1);
            }
            String valueResult;
            if (!webTypeInfoString.equals("") && !accumualteInfo.equals("")){
                valueResult = webTypeInfoString + Properties.Base.CTRL_B + accumualteInfo;
            } else if (!webTypeInfoString.equals("") && accumualteInfo.equals("")){
                valueResult = webTypeInfoString;
            } else if (webTypeInfoString.equals("") && !accumualteInfo.equals("")){
    ...
    public static void main(String[] args) throws Exception{
        ...
        Job job = new Job(conf,"AccumulateWebType");
...
//        job.getConfiguration().set("mapreduce.map.java.opts ", "-Djava.net.preferIPv4Stack=true -Xmx3000000000");
        ...
        job.setJarByClass(AccumulateWebType.class);
        ...
        for (int i=0;i<1;i++) {
            CommUtil.addInputFileComm(job, fs, bidLog + "/" + currentData + "/WebType", TextInputFormat.class, SingleDayMapper.class);
